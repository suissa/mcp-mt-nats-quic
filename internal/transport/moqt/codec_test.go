...
func TestToolCallAndResultObjectMapping(t *testing.T) {
	...
	if req.Namespace.String() != "(mcp, s1, tools)" || req.Track != "nats_info" || req.Event != EventToolsCall || req.GroupID != 7 || req.ObjectID != 0 {
		t.Fatalf("bad req: %#v", req)
	}
	res := ResultObject("s1", "nats_info", 7, 1, []byte(`{"jsonrpc":"2.0"}`))
	if res.Event != EventToolsResult || res.ObjectID != 1 {
